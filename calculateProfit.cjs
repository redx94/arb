...
const profitCalculator_ts_1 = require("./src/utils/arbitrage/profitCalculator.js");
const priceFeeds_ts_1 = require("./src/utils/priceFeeds.js");
const ethers_1 = require("ethers");
const { FlashLoanHandler } = require('./src/utils/flashLoanHandler.ts');
async function main() {
    const profitCalculator = profitCalculator_ts_1.ProfitCalculator.getInstance();
    const priceFeed = priceFeeds_ts_1.PriceFeed.getInstance();
    const currentPriceData = await priceFeed.getCurrentPrice();
    const buyPrice = currentPriceData.price;
    const sellPrice = currentPriceData.price * 1.02;
    const amount = 1;
    ...
    const netProfitETH = ethers_1.ethers.formatEther(netProfitWei);
    const ethPriceInUSD = currentPriceData.price;
    const netProfitUSD = netProfitETH * ethPriceInUSD;
